...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `active_fclist` AS select 2018 AS `Fiscal Year`,`f`.`Brand` AS `Brand`,`f`.`DVCode` AS `DVCode`,`f`.`FCDesc` AS `FCDesc`,`f`.`LY FC` AS `LY FC` from `tblfclist` `f` where (((`f`.`Fiscal Year` >= 2018) or (`f`.`Fiscal Year` = 9999)) and (`f`.`Brand` <> 'unassigned')) union select 2018 AS `Fiscal Year`,`tblfclist`.`Brand` AS `brand`,`tblfclist`.`FC Group` AS `FC Group`,`tblfclist`.`FC Group` AS `FC Group`,`tblfclist`.`FC Group` AS `FC Group` from `tblfclist` where ((`tblfclist`.`FC Group` is not null) and (`tblfclist`.`FC Group` <> 'current') and (`tblfclist`.`FC Group` <> 'Winter 16') and (`tblfclist`.`Brand` <> 'unassigned')) group by `tblfclist`.`Brand`,`tblfclist`.`FC Group` order by `Brand`,`DVCode`,`FCDesc` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
--
-- Dumping events for database 'feature-code2'
--
...
-- Dump completed on 2018-07-02  8:36:39
